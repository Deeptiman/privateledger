...
const (
	...
	ccPath      = "privateledger/chaincode/"
	ccId        = "CC_ORG_V00"
	ccVersion   = "1"
...
func getArtifactPath() string {
	return os.Getenv("GOPATH") + "/src/privateledger/fixtures/artifacts/"
}
...
func getCollectionConfigs() []*pr.CollectionConfig {
	return collConfigs
}
...
func newCollectionConfig(colName, policy string, reqPeerCount, maxPeerCount int32, blockToLive uint64) (*pr.CollectionConfig, error) {
	...
	cpc := &pr.CollectionPolicyConfig{
		Payload: &pr.CollectionPolicyConfig_SignaturePolicy{
			SignaturePolicy: p,
		},
	}
	return &pr.CollectionConfig{
		Payload: &pr.CollectionConfig_StaticCollectionConfig{
			StaticCollectionConfig: &pr.StaticCollectionConfig{
				Name:              colName,
